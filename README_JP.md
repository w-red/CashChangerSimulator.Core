...
本リポジトリでは、以下の 3 つのパッケージを管理しています：
...
> [!NOTE]
> ユーザーインターフェース・コンポーネントは、それぞれの専用のリポジトリで管理されています：
> [**Cli** (コマンドライン・インターフェース)](https://github.com/w-red/CashChangerSimulator.Cli) および [**Wpf** (Windows デスクトップ UI)](https://github.com/w-red/CashChangerSimulator.Wpf)
...
- **UPOS 準拠の動作**: 仮想デバイス層を通じて `DispenseChange`、`DispenseCash`、および入金サイクルを正確にエミュレート。
...
- [API 仕様書](docs/ApiSpecification_JP.md): POS for .NET およびネイティブ C# 向けの API リファレンス。
- [UPOS Compliance Mapping](docs/UposComplianceMapping_JP.md): UPOS インターフェースの実装状況。
