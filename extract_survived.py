...
def find_json_in_html(file_path):
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
        for search_str in [".report =", "const report =", '{"schemaVersion"']:
            start_idx = content.find(search_str)
            if start_idx != -1: break
        if start_idx == -1: return None
        if not content[start_idx] == '{':
...
if data:
    for f, file_data in data.get('files', {}).items():
        if "DepositController.cs" in f:
            ...
            print(f"File: {f}")
            print(f"Total mutants in file: {len(file_data.get('mutants', []))}")
            print(f"Survived mutants: {len(survived)}")
            for m in survived:
                loc = m['location']['start']
                print(f"  Line {loc['line']}:{loc['column']} | {m['mutatorName']} -> {m['replacement']}")
else:
    print("No data found.")
