...
/// <summary>HardwareStatusManager の各状態遷移、エラー設定、およびグローバルロック、排他制御を検証するテストクラス。</summary>
public class HardwareStatusManagerTests
{
    private readonly HardwareStatusManager manager = HardwareStatusManager.Create();

    /// <summary>詰まり状態と発生箇所を正しく設定・取得できることを検証する。</summary>
    [Fact]
    ...
    /// <summary>回収箱の取り外し状態を設定・取得できることを検証する。</summary>
    [Fact]
    ...
    /// <summary>ResetTrigger ですべてのエラー状態がクリアされることを検証する。</summary>
    [Fact]
    ...
    /// <summary>GlobalLockManager との統合動作を検証する。</summary>
    [Fact]
    ...
    /// <summary>ステータス更新イベントが正しく通知されることを検証します。</summary>
    [Fact]
    ...
    /// <summary>破棄(Dispose)後の状態設定で例外が発生することを検証する(R3 の標準動作)。</summary>
    [Fact]
    public void AfterDisposeOperationsShouldThrow()
    {
        ...
        // R3 の ReactiveProperty は Dispose 済みだと Value への操作で Object Disposed Exception を投げる
