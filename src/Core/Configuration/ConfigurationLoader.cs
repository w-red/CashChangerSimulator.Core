...
public static class ConfigurationLoader
{
    ...
    public static string DefaultConfigFilePath => DefaultConfigPath;

    /// <summary>Gets デフォルトの在庫状態保存先ファイルパス。.</summary>
    public static string DefaultInventoryStateFilePath => InventoryStatePath;

    /// <summary>Gets デフォルトの取引履歴保存先ファイルパス。.</summary>
    public static string DefaultHistoryStateFilePath => HistoryStatePath;
    ...
    public static SimulatorConfiguration Load(string? path = null)
    {
        ...
        {
            Logger.ZLogError(ex, $"IO Error loading configuration from {filePath}. Returning default.");
            return new SimulatorConfiguration();
        }
        catch (UnauthorizedAccessException ex)
        {
            Logger.ZLogError(ex, $"Access denied loading configuration from {filePath}. Returning default.");
            return new SimulatorConfiguration();
        }
        catch (TomlException ex)
        {
            Logger.ZLogError(ex, $"Invalid TOML format in {filePath}. Returning default.");
            return new SimulatorConfiguration();
        }
    }
    ...
    public static InventoryState LoadInventoryState(string? path = null)
    {
        ...
        {
            Logger.ZLogError(ex, $"IO Error loading inventory state from {filePath}. Returning empty state.");
            return new InventoryState();
        }
        catch (UnauthorizedAccessException ex)
        {
            Logger.ZLogError(ex, $"Access denied loading inventory state from {filePath}. Returning empty state.");
            return new InventoryState();
        }
        catch (TomlException ex)
        {
            Logger.ZLogError(ex, $"Invalid TOML format in inventory state {filePath}. Returning empty state.");
            return new InventoryState();
        }
    }
